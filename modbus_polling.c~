/////////////////////////////////////////////////////////////////////////////////////////////
// modbus_polling.c - 轮询RS485总线上的设备，包括切换器和充电机
//
//
////////////////////////////////////////////////////////////////////////////////////////////
#include <unistd.h>
...
T_MODBUS_PARA _modbus;//MODBUS通信管理所需的数据全包含在这个结构中
T_MODBUS_ERROR modbus_error;

//modbus总线UART(RS485)端口
int fd_modbus;
...
int switcher_sm;//切换器状态机
int charger_sm;//充电机状态机
int charger_channel;//充电通道

//获取切换器的状态，包括电池电压等
//IN: timeout_ms 等待时间[ms]，最小分辨率为10ms
//OUT: 如果返回值为1，则data 返回电池状态，否则返回数据无效

//modbus接收线程，用于在RS485总线上接收所有modbus数据
void modbus_recv_thread(void);
//modbus轮询线程，由主线程中的信号量控制轮询周期
...
int modbus_init()
{
	//MODBUS总线上的数据记录全部放在全局记录中,因此，在初始化时不再新创建记录文件
	...
	pthread_create(&modbus_recv_thread_id, NULL, (void*)modbus_recv_thread, NULL);//RS485 接收线程
	pthread_create(&modbus_master_thread_id, NULL, (void*)modbus_master_thread, NULL);//Modbus 主控线程
	...
	while (1)
	{
		...
		for (i = 0; i < nread; i++)
		{
			...
			case SM_MODBUS_RECV_DATA:
				...
				if (_modbus.recv_cnt >= _modbus.recv_len)//recv_len:不含CRC的包长度
				{
			...
			case SM_MODBUS_RECV_CRC2:
				...
				if ((_crc[0] == ((crc_tmp >> 8) & 0xff)) && (_crc[1] == (crc_tmp & 0xff)))//解析正确，可处理数据
				{
...
int modbus_get_switcher_state(T_SWITCHER_DATA* data, int timeout_ms)
{
	...
	//发送格式：从站地址(1B) | 功能号(1B) | 寄存器起始地址(2B) | 读取的寄存器数(2B) | CRC
	//此处： SWITCHER_ADDR | 0x03 | 0x0, 0x0 | 0x0,0x4 | CRCH,CRCL
	buf[0] = SWITCHER_ADDR;//从站地址
	buf[1] = 0x03;//功能号
	buf[2] = 0x0;
	buf[3] = 0x0;//[2][3]:起始寄存器地址
	buf[4] = 0x0;
	buf[5] = 0x4;//[4][5]:读取的寄存器数（每个寄存器2个字节），因此此处为读取8个字节
	crc_tmp = crc16(buf, 6);
...
*/
	...
	//预期返回：从站地址(1B) | 功能号(1B) | 返回数据的字节数(1B) | 8字节的数据 | CRCH,CRCL ，不算CRC一共11个字节
	//此处：SWITCHER_ADDR | 0x03 | 0x08 | 4*2字节的数据,每个寄存器高位在前 | CRCH,CRCL
	_modbus.recv_addr = SWITCHER_ADDR;
	_modbus.recv_flag = 0x03;
	_modbus.recv_len = 11;//返回11个字节（不含CRC）
	_modbus.recv_cnt = 0;
	_modbus.recv_ok = 0;

	_timeout_cnt = (int)(0.1 * timeout_ms);//将毫秒转换为sleep次数
...
int modbus_set_switcher(unsigned short addr, unsigned short value, int timeout_ms)
{
	...
	//发送格式：从站地址(1B) | 功能号(1B) | 寄存器起始地址(2B) | 寄存器的设置值(2B) | CRC
	//此处： SWITCHER_ADDR | 0x06 | addrH,addrL | valueH,valueL | CRCH,CRCL
	buf[0] = SWITCHER_ADDR;
	buf[1] = 0x06;//设置命令
	buf[2] = 0x0;
	buf[3] = (unsigned char)addr;//切换器寄存器地址
...
/*
	...
	_DBG_0("\n");
*/
	//预期返回与发送数据相同：从站地址(1B) | 功能号(1B) | 寄存器起始地址(2B) | 寄存器的设置值(2B) | CRC
	_modbus.recv_addr = SWITCHER_ADDR;
	_modbus.recv_flag = 0x06;
	_modbus.recv_len = 6;//返回6个字节（不含CRC）
	_modbus.recv_cnt = 0;
	_modbus.recv_ok = 0;
...
int modbus_get_charger_state(T_CHARGER_DATA* data, int timeout_ms)
{
	...
	//发送格式：从站地址(1B) | 功能号(1B) | 寄存器起始地址(2B) | 读取的寄存器数(2B) | CRC
	//此处： CHARGER_ADDR | 0x03 | 0x0, 0x0 | 0x0,0x4 | CRCH,CRCL
	buf[0] = CHARGER_ADDR;
	buf[1] = 0x03;
	buf[2] = 0x0;
	buf[3] = 0x0;
...
*/
	//预期返回：从站地址(1B) | 功能号(1B) | 返回数据的字节数(1B) | 8字节的数据 | CRCH,CRCL ，不算CRC一共11个字节
	//此处：CHARGER_ADDR | 0x03 | 0x08 | 4*2字节的数据,每个寄存器高位在前 | CRCH,CRCL
...
int modbus_set_charger(unsigned short addr, unsigned short value, int timeout_ms)
{
	...
	//发送格式：从站地址(1B) | 功能号(1B) | 寄存器起始地址(2B) | 寄存器的设置值(2B) | CRC
	//此处： CHARGER_ADDR | 0x06 | addrH,addrL | valueH,valueL | CRCH,CRCL
	buf[0] = CHARGER_ADDR;
	buf[1] = 0x06;
	buf[2] = 0x0;
	buf[3] = (unsigned char)addr;//切换器寄存器地址
...
*/
	...
	case SM_CHARGER_SET_CURRENT:
		...
		ret = modbus_set_charger(0x11, us_tmp, MODBUS_TIMEOUT_MS);//设置电流:向地址0x10写电流，单位[0.1A]
		if (ret > 0)
		{
	...
	case SM_CHARGER_TURNON:
		ret = modbus_set_charger(0x12, 0, MODBUS_TIMEOUT_MS);//开机
		if (ret > 0)
		{
		...
		break;
	case SM_CHARGER_QUARY://充电中的状态查询
