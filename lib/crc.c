//////////////////////////////////////////////////////////////////////////
// crc_check.cpp - CRC校验值计算函数,参照资料数据
//
// Author:  Fei Qing
// Created: Feb 11, 2016
//
// Modification History
// --------------------
//////////////////////////////////////////////////////////////////////////
#include "crc.h"

unsigned char auchCRCHi[] = {
};

unsigned char auchCRCLo[] = {
};

///////////////////////////////////////////////////////////
//函数名：Modbus_crc16     
//功能： modbus代码校验      
//参数： 字符型指针或数组首地址，数据个数     
//返回值： crc数据     
//备注：在收到和发送的过程中都需要调用。  
///////////////////////////////////////////////////////////
unsigned short crc16(unsigned char *ptr,unsigned int len)
{
    unsigned char uchCRCHi=0xFF;  /* 高CRC字节初始化 */ 
    unsigned char uchCRCLo=0xFF;  /* 低CRC 字节初始化 */ 
    unsigned long uIndex;         /* CRC循环中的索引 */
/*
	int i;
	printf("CRC: ");
	for(i=0;i<len;i++) printf("0x%x,",ptr[i]);
	printf("\n");
*/
    while(len--)
    {
        uIndex=uchCRCHi^*ptr++;
        uchCRCHi=uchCRCLo^auchCRCHi[uIndex];
        uchCRCLo=auchCRCLo[uIndex];
    }
    return (uchCRCHi<<8|uchCRCLo);
}
///////////////////////////////////////////////////
//计算CRC码的步骤为：
//(1).预置16位寄存器为FFFFH。称此寄存器为CRC寄存器；
//(2).把第一个8位数据与CRC寄存器的低位相异或，把结果放于CRC寄存器；
//(3).把寄存器的内容右移一位(朝低位)，用0填补最高位，检查最低位；
//(4).如果最低位为0：重复第3步(再次移位)如果最低位为1：CRC寄存器与多项式A001进行异或；
//(5).重复步骤3和4，直到右移8次，这样整个8位数据全部进行了处理；
//(6).重复步骤2到步骤5，进行下一个8位数据的处理；
//(7).最后得到的CRC寄存器即为CRC码。(CRC码 = CRC_L +CRC_H)
///////////////////////////////////////////////////
unsigned short crc16_2(unsigned char *ptr,unsigned int len)
{
  unsigned short wCRC = 0xffff;//预置16位crc寄存器
  unsigned char temp;
  int i=0,j=0;

  for(i=0;i<len;i++)
  {
    temp = *ptr&0xff;
    wCRC ^= temp;//将1个字节的数据与crc寄存器异或
    ptr++;

    for(j=0;j<8;j++)
    {
      if(wCRC&0x01)//判断右移出的是不是1，如果是1则与多项式进行异或。
      {
        wCRC >>= 1;
        wCRC ^= 0xa001;//与上面的多项式进行异或
      }
      else
      {
        wCRC >>= 1;
      }
    }
  }
  return (((wCRC<<8)&0xff00) | ((wCRC>>8) & 0xff));
}

unsigned short crc16_3(unsigned char *ptr,unsigned int len)
{
	unsigned char crch=0xff; 
	unsigned char crcl=0xff; 
	unsigned char index;
	int i;
/*
	printf("CRC3: ");
	for(i=0;i<len;i++) printf("0x%x,",ptr[i]);
	printf("\n");
*/
	for(i=0;i<len;i++)
	{
		index=crch^ptr[i];
		crch=crcl^auchCRCHi[index];
		crcl=auchCRCLo[index];
	}
	//printf("CRC3: res: 0x%x,0x%x\n",crch,crcl);
	return (crch<<8|crcl);
}
